...
class ForaW310 {
    ...
    private static let CMD_DATA: [UInt8] = [81, 113, 2, 1, 0, 163, 104]
...
//        }
    ...
    private func setDiscoverObserver() {
        self.discoverDisposable = self.peripheral
            .flatMap { (peripheral: Peripheral) -> Observable<[Service]> in
                return peripheral.discoverServices([CBUUID(string: ForaW310.SERVICE_UUID)]).asObservable()
        }.flatMap({ (services: [Service]) -> Observable<[Characteristic]> in
            return services[0].discoverCharacteristics([CBUUID(string: ForaW310.CHAR_UUID)]).asObservable()
        })
    ...
    private func setNotifyObserver() {
        self.notifyDisposable = self.characteristic
            ...
            .subscribe(onNext: { (chracteristic) in
                ...
                if self.count == 4 {
                    guard let mac = self.macAddress else {
                        self.dataStream.onNext(DeviceEvent(suc: false, data: nil))
                        return
                    }
                    let d = ForaW310Data.parse(name: self.advertisementData.localName ?? "", macAddress: mac, data: self.data)
                    if d == nil {
                        self.dataStream.onNext(DeviceEvent(suc: false, data: nil))
                    } else {
    ...
    private func setWriteCmdObserver() {
        self.writeDisposable = self.characteristic
            ...
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaW310.CMD_DATA), type: .withResponse).asObservable()
            })
            .delay(.milliseconds(1000), scheduler: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaW310.CMD_TURN_OFF), type: .withResponse).asObservable()
            })
