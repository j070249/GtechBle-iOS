...
class ForaP60 {
    ...
    private func setDiscoverObserver() {
        self.discoverDisposable = self.peripheral
            .flatMap { (peripheral: Peripheral) -> Observable<[Service]> in
                return peripheral.discoverServices([CBUUID(string: ForaP60.SERVICE_UUID)]).asObservable()
        }.flatMap({ (services: [Service]) -> Observable<[Characteristic]> in
            return services[0].discoverCharacteristics([CBUUID(string: ForaP60.CHAR_UUID)]).asObservable()
        })
        ...
        self.notifyDisposable = self.characteristic
            ...
            .subscribe(onNext: { (chracteristic) in
                ...
                if self.count == 2 {
                    ...
                    let d = ForaP60Data.parse(name: self.advertisementData.localName ?? "", macAddress: mac, time: time, data: data)
    ...
    private func setWriteCmdObserver() {
        self.writeDisposable = self.characteristic
            ...
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaP60.CMD_TIME), type: .withResponse).asObservable()
            })
            .delay(.milliseconds(200), scheduler: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaP60.CMD_DATA), type: .withResponse).asObservable()
            })
            .delay(.milliseconds(200), scheduler: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaP60.CMD_TURN_OFF), type: .withResponse).asObservable()
            })
