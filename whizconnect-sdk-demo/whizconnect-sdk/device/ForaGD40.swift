...
class ForaGD40 {
    ...
    private func setDiscoverObserver() {
        self.discoverDisposable = self.peripheral
            .flatMap { (peripheral: Peripheral) -> Observable<[Service]> in
                return peripheral.discoverServices([CBUUID(string: ForaGD40.SERVICE_UUID)]).asObservable()
        }.flatMap({ (services: [Service]) -> Observable<[Characteristic]> in
            return services[0].discoverCharacteristics([CBUUID(string: ForaGD40.CHAR_UUID)]).asObservable()
        })
        ...
        self.notifyDisposable = self.characteristic
            ...
            .subscribe(onNext: { (chracteristic) in
                ...
                if self.count == 2 {
                    ...
                    let d = ForaGD40Data.parse(name: self.advertisementData.localName ?? "", macAddress: mac, time: time, data: data)
    ...
    private func setWriteCmdObserver() {
        self.writeDisposable = self.characteristic
            ...
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaGD40.CMD_TIME), type: .withResponse).asObservable()
            })
            .delay(.milliseconds(200), scheduler: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaGD40.CMD_DATA), type: .withResponse).asObservable()
            })
            .delay(.milliseconds(200), scheduler: ConcurrentDispatchQueueScheduler(qos: .userInitiated))
            .flatMap({ (char: Characteristic) -> Observable<Characteristic> in
                return char.writeValue(Data(ForaGD40.CMD_TURN_OFF), type: .withResponse).asObservable()
            })
